...
| `--direct` | **Direct multi-horizon head**: predicts all `pred_len` corrections at once from the pooled context (no autoregressive rollout), each anchored at seed constant-velocity `p0+(s+1)·v0` (fc2 zero-init → untrained = seed-CV). Avoids rollout error compounding. **Measured the roadmap's biggest win** — better than `--cv` at 6h (32.1 vs 35.2, ties CLIPER) *and* 48h (−10 km 6–48h mean, 5/5 seeds), tighter variance, and faster (FINDINGS §22). Excludes `--cv`/`--gru`/`--xattn`. | off |
...
| `--km_loss` | **Equirectangular kilometre objective**: scales the longitude residual by `(cstd_lon/cstd_lat)·cos(lat)` so the loss ≈ squared km error (aligns the objective with the ΔR metric). Consistent (loss + gradient), gradient-checked, and applied on **both** training paths (serial and `--threads=N`). Tested 5-seed — **measured neutral** at 6h and 48h (FINDINGS §21); off by default. | off |
