...
- [ ] **Scale past 98 storms — the ceiling on everything.** The tooling is now in place: download HURDAT2 from https://www.nhc.noaa.gov/data/#hurdat (unreachable from this dev environment's network), run `python tools/hurdat2_to_csv.py hurdat2-*.txt ../HURDAT_full.csv` (default `--since=2004`, ~350 storms with wind radii), then train text-free: `./typhoformer train 30 --motion --physics --cv --csv=../HURDAT_full.csv --emb=none --threads=N`. Re-run the FINDINGS grid; this is what resolves every "within noise" question in §7–§13. (Text embeddings only needed if re-testing the language branch: `tools/gen_descriptions.py` + `gen_embeddings.py`.)
