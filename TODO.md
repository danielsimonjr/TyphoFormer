...
- [ ] 🟢 **NEXT — re-run the FINDINGS grid on the real data.** This is the actual payoff, and it is now unblocked. Every "within noise" verdict in §7–§14 was measured on **98 storms**; with 826 the noise floor drops and those questions become answerable. Start with the best-known recipe:
      `./typhoformer train 30 --patience=8 --motion --physics --cv --huber=0.1 --csv=../HURDAT_full.csv --emb=none --threads=N`
      Then re-test, at minimum, the results that were *negative or marginal* on the small sample and could plausibly flip with 8× the data: the `--gru`/`--xattn` long-horizon decoders (§11), `--rotframe` (§13), and the language branch (§15 — `--no_text` being *better* is exactly the kind of result a tiny sample can manufacture). **Treat every prior FINDINGS number as provisional until re-measured here** — the sample size was the confound in most of them.
