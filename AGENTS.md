...
Guidance for AI coding agents working in this repository. Keep this file and `CLAUDE.md` in sync — they carry the same guidance.
...
./typhoformer train 30 --motion --physics --direct --huber=0.1 --save=m.ckpt   # the best-known recipe (works serial or --threads=N)
...
`typhoformer-c/docs/FINDINGS.md` is the honest experimental record. Key conclusions: the default input featurization is blind to motion (intensity + text only), so train with `--motion --physics --direct --huber=0.1` (works serial or `--threads=N`). The `--direct` multi-horizon head (§22) is the current best decoder — it beats the older autoregressive `--cv` head at **every** horizon (32.1 vs 35.2 km @ 6h — level with CLIPER — and −10 km over the 6–48h rollout, 5/5 seeds) and trains ~1.2× faster; §1–§21 were all measured on `--cv`. On the full 826-storm dataset (`--csv=../HURDAT_full.csv --emb=none`) the recipe beats split-matched constant-velocity **at every horizon** (−17.2 km over 6–48h; FINDINGS §22); on the bundled 98-storm sample it scores 38.5 km @ 6h, under that sample's 39.4 km bar; the language branch does **not** help on this data (`--no_text` is marginally better); the honest baseline is constant-velocity (~39 km @ 6h), not persistence; decoder memory (`--gru`/`--xattn`) did not survive a five-split re-test (§11) — single-split results on 98 storms can flip under float-rounding-level perturbations, so always test across multiple `--split_seed`s. An earlier "beats persistence" result was data leakage — the storm-level split and train-only stats exist to prevent that regression. Report held-out test numbers, not validation numbers.
