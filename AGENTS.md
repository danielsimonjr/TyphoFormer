...
./typhoformer train 30 --motion --physics --cv --huber=0.1 --save=m.ckpt   # the best-known recipe (works serial or --threads=N)
...
`typhoformer-c/docs/FINDINGS.md` is the honest experimental record. Key conclusions: the default input featurization is blind to motion (intensity + text only), so train with `--motion --physics --cv --huber=0.1` (works serial or `--threads=N`). On the full 826-storm dataset (`--csv=../HURDAT_full.csv --emb=none`) the recipe beats split-matched constant-velocity over the 48h rollout on all five splits (−8.4% @ 48h; 6h dead reckoning stays ahead — FINDINGS §15); on the bundled 98-storm sample it scores 38.5 km @ 6h, under that sample's 39.4 km bar; the language branch does **not** help on this data (`--no_text` is marginally better); the honest baseline is constant-velocity (~39 km @ 6h), not persistence; decoder memory (`--gru`/`--xattn`) did not survive a five-split re-test (§11) — single-split results on 98 storms can flip under float-rounding-level perturbations, so always test across multiple `--split_seed`s. An earlier "beats persistence" result was data leakage — the storm-level split and train-only stats exist to prevent that regression. Report held-out test numbers, not validation numbers.
