...
- **📈 Trains end to end, measured honestly.** A **leakage-safe** pipeline (storm-level train/val/**test** split, train-only normalization) with dropout, AdamW, gradient clipping, and LR warmup. The default model turned out to be **blind to motion** (its inputs were intensity + text; position/velocity were never fed in), so it lost to a two-line constant-velocity baseline. Adding motion features (`--motion`) and predicting displacement (`--delta`) takes held-out ΔR from **128 → 48 km** — 2.5× better than persistence and competitive with constant-velocity (~39 km). A `--no_text` ablation shows the **language branch still doesn't help** even with a working model. Full write-up: [`docs/FINDINGS.md`](typhoformer-c/docs/FINDINGS.md).
