...
- **⚙️ Optimized math.** Hand-written, **cache-aware** matrix multiplies (`ikj` / `pij` loop orders, unrolled multi-accumulator reductions that auto-vectorize at plain `-O3`) — ≈ 1 s/epoch for the compact demo config and ≈ 23 s/epoch for the full config (2.77 M params; the degenerate spatial blocks are off by default, `--spatial` restores the paper's 5.14 M) on a 4-core container; `make NATIVE=1` / `make MARCH=x86-64-v3` (SIMD) and `--threads=N` (data-parallel multicore) scale it further.
...
- **📈 Trains end to end, measured honestly.** A **leakage-safe** pipeline (storm-level train/val/**test** split, train-only normalization) with dropout, AdamW, gradient clipping, and LR warmup. The default model turned out to be **blind to motion** (its inputs were intensity + text; position/velocity were never fed in), so it lost to a two-line constant-velocity baseline. The best-known recipe (`--motion --physics --direct --huber=0.1`) takes held-out ΔR from **131 → 38.5 km** on the bundled sample, and trained on the full three-basin dataset (**826 storms**, fetched by `tools/fetch_hurdat2.sh`, no text embeddings needed) it **beats split-matched constant-velocity at every horizon on all five storm splits** — 32.1 km @ 6h (level with CLIPER) to −17.2 km over the 6–48h rollout — using the direct multi-horizon head (§22), which also trains ~1.2× faster than the older autoregressive `--cv` head that every result before §22 was measured on. A `--no_text` ablation shows the **language branch still doesn't help** even with a working model. Full write-up: [`docs/FINDINGS.md`](typhoformer-c/docs/FINDINGS.md).
