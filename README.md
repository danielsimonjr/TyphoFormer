...
- **📈 Trains end to end, measured honestly.** A **leakage-safe** pipeline (storm-level train/val/**test** split, train-only normalization) with dropout, AdamW, gradient clipping, and LR warmup. Across five storm splits the model is at **rough parity with persistence** (test ΔR **128 ± 41 km** vs 123 km — the split noise dwarfs the gap), and a `--no_text` ablation shows the **language branch does not help** on this data (numbers-only is ~17 km *better*). The earlier "79 km beats persistence" was a data-leakage artifact, now fixed. Full write-up: [`docs/FINDINGS.md`](typhoformer-c/docs/FINDINGS.md).
