...
- **📈 Trains end to end, measured honestly.** A **leakage-safe** pipeline (storm-level train/val/**test** split, train-only normalization) with dropout, AdamW, gradient clipping, and LR warmup. The default model turned out to be **blind to motion** (its inputs were intensity + text; position/velocity were never fed in), so it lost to a two-line constant-velocity baseline. The best-known recipe (`--motion --physics --cv --huber=0.1`) takes held-out ΔR from **131 → 38.5 km** on the bundled sample, and trained on the full three-basin dataset (**826 storms**, fetched by `tools/fetch_hurdat2.sh`, no text embeddings needed) it **beats split-matched constant-velocity over the 48-hour rollout on all five storm splits** (crossover at 24h, −8.4% at 48h; one-step dead reckoning stays ahead at 6h). A `--no_text` ablation shows the **language branch still doesn't help** even with a working model. Full write-up: [`docs/FINDINGS.md`](typhoformer-c/docs/FINDINGS.md).
