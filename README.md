...
- **📈 Trains end to end, measured honestly.** A **leakage-safe** pipeline (storm-level train/val/**test** split, train-only normalization) with dropout, AdamW, gradient clipping, and LR warmup. The default model turned out to be **blind to motion** (its inputs were intensity + text; position/velocity were never fed in), so it lost to a two-line constant-velocity baseline. Adding motion features (`--motion`) and predicting displacement (`--delta`) takes held-out ΔR from **131 → 48 km** — 2.5× better than persistence — and anchoring the decoder at constant-velocity (`--cv`) reaches **40.8 km**, matching the CLIPER baseline (~39 km) at 6h and **beating it at 48h** on 4 of 5 storm splits. A `--no_text` ablation shows the **language branch still doesn't help** even with a working model. Full write-up: [`docs/FINDINGS.md`](typhoformer-c/docs/FINDINGS.md).
