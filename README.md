...
- **📈 Trains end to end, measured honestly.** A **leakage-safe** pipeline (storm-level train/val/**test** split, train-only normalization) with dropout, AdamW, gradient clipping, and LR warmup. On truly held-out storms the compact demo trains stably but does **not** beat persistence (test ΔR ≈ 173 km vs ≈ 103 km) — the earlier "79 km" figure was a data-leakage artifact, now fixed. Per-horizon (6h/12h/…) metrics and in-repo persistence / CLIPER baselines included.
