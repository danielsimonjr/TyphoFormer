...
### Added
- `tools/fetch_hurdat2.sh`: one-command dataset scaling — discovers the newest Atlantic and NE Pacific HURDAT2 files from NOAA's directory listing, downloads both, converts via `hurdat2_to_csv.py`, and merges into `HURDAT_full.csv` ready for text-free recipe training. (The download itself is blocked from this dev environment's network policy — NOAA 403s on every pathway — so the script is verified on synthetic two-basin data; run it wherever NOAA is reachable.)
