...
- Unrolled/contiguous restructuring of the remaining reduction loops: `linear_backward`'s bias reduction and all six strided single-accumulator loops in `mha_forward`/`mha_backward`. Full-config bench (portable, with the new default): forward 4.10 ms/sample, forward+backward 16.6 ms — vs 10.8/39.6 before this round and 60.3/84.0 at the start of the optimization work. Golden re-pinned (0.02700 → 0.02507) for the intentional architecture-default + summation-order change.
